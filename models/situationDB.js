...
//상황 저장
exports.insertSituation = async (body,image)=> {
    try{
        console.log(body)
        const conn = await pool.getConnection(async(conn) => conn);
        const {image,level,answer,question} = body;
        await conn.query(`INSERT INTO situation (level,image,answer,question) VALUES ("${level}","${image}","${answer}","${question}")`);
        conn.release();
...
//상황 정보 가져오기
exports.selectSituation = async ()=> {
    try{
        const conn = await pool.getConnection(async(conn) => conn);
        const list = await conn.query(`SELECT * FROM situation"`);
        console.log(list)
        conn.release();
        return list[0][0];
    }catch(err){
...
//상황 선택 저장
exports.insertSituationSelect = async (body)=> {
    try{
        console.log(body)
        const conn = await pool.getConnection(async(conn) => conn);
        const {situation_num,firstNum,secondNum,thirdNum,fourthNum} = body;
